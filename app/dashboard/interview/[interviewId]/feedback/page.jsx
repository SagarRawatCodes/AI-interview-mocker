...
function Feedback({ params }) {
  ...
  const GetFeedback = async () => {
    ...
    if (result.length > 0) {
      const total = result.reduce((acc, item) => acc + (item.rating || 0), 0);
      const average = total / result.length;
      setAverageRating(average.toFixed(1)); // Round to 1 decimal
    }
  };
  ...
  return (
    <div className="p-10">
      ...
      <h2 className="text-primary text-lg my-3">
        Your interview rating: <strong>{averageRating !== null ? `${averageRating}/10` : "__/10"}</strong>
      </h2>
...
function Feedback({params}) {
  ...
  return (
    <div className="p-10">
        <h2 className="text-3xl font-bold text-green-500">Congratulations!</h2>
        <h2 className="font-bold  text-2xl">Here is your interview feedback</h2>
        <h2 className="text-primary text-lg my-3">Your interview rating: <strong>__/10</strong></h2>
